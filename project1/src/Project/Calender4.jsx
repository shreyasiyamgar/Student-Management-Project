...
const Calendar4 = () => {
  ...
  const startDay = new Date(2025, 9, 1).getDay(); // October 1, 2025
  ...
  return (
    <div className="w-full min-h-screen bg-white font-sans p-4 sm:p-6 md:p-8">
      ...
      <div className="flex justify-between items-center mt-8 mb-4">
        <button className="text-4xl text-gray-700" onClick={() => navigate('/Calender3')}>{'<'}</button>
        <h3 className="text-2xl sm:text-4xl font-semibold">October, 2025</h3>
        <button className="text-4xl text-gray-700" onClick={() => navigate('/Calender5')}>{'>'}</button>
      </div>
      ...
      <div className="grid grid-cols-7 text-center bg-indigo-100 text-black py-2 sm:py-3 text-base sm:text-lg rounded-b-md">
        ...
        {Array.from({ length: 31 }, (_, i) => {
          const day = i + 1;
          const isEvent =
            day === 2 || (day >= 17 && day <= 31); // Gandhi Jayanti + Diwali Vacations
          return (
      ...
      <div className="flex flex-col gap-4 bg-indigo-100 mt-6 p-4 rounded-md w-full max-w-8xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-pink-200 text-black rounded-lg p-2 w-16 text-center">
            ...
            <div className="text-lg sm:text-xl lowercase">oct</div>
          </div>
          <div className="text-lg sm:text-2xl font-medium">Mahatma Gandhi Jayanti</div>
        </div>
        {/* Event 2 */}
        <div className="flex items-center gap-4">
          <div className="bg-pink-200 text-black rounded-lg p-2 w-20 text-center">
            <div className="text-sm sm:text-lg font-bold">17–31</div>
            <div className="text-sm sm:text-base lowercase">oct</div>
          </div>
          <div className="text-lg sm:text-2xl font-medium">Diwali Vacations</div>
        </div>
      </div>
    </div>
  );
